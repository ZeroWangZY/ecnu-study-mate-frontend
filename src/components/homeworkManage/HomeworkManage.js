...
class HomeworkManage extends Component {
  ...
  openEditDialog = event => {
    this.setState({
      ...
      deadline: event.deadline.toString().substr(0, 10) + 'T' + event.deadline.toString().substr(11, 5),
    ...
    return (
      <div className="homeworkManage-container" style={{ textAlign: 'center' }}>
        {this.props.homeList.map((item, i) => {
          return (
            <HomeworkManageItem
              key={item.homeworkID + currentTime}
              item={item}
              openEditDialog={this.openEditDialog.bind(this)}
              openDeleteDialog={this.openDeleteDialog.bind(this)}
            />
          )
        })}
        <div className="homeworkManage-addButton">
          ...
          >
            <DialogTitle id="alert-dialog-slide-title">
              {this.state.isDelete ? '删除作业' : this.state.editing ? '编辑作业' : '新增作业'}
            </DialogTitle>
...
const mapDispatchToProps = dispatch => ({
  ...
  addHomework: (title, content1, deadline, publisher, receiver) =>
    dispatch(addHomework(title, content1, deadline, publisher, receiver)),
  deleteHomework: homeworkID => dispatch(deleteHomework(homeworkID)),
  editHomework: (homeworkID, title, content1, deadline, receiver, grade, state) =>
    dispatch(updateHomework(homeworkID, title, content1, deadline, receiver, grade, state))
})
